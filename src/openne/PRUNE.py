...
class PRUNE(object):
    ...
    def __init__(self, graph, dim=128, lamb=0.01, learning_rate=1e-4, epoch=50, gpu_fraction=0.20, batchsize=1024, print_every_epoch=1, save_checkpoints=True):
        self.size = dim
        self.graph = graph

        nodeCount = graph.node_size
        edgeArray = graph.edge_array()

        # PRUNE: node embeddings
        embeddings = run_PRUNE(lamb, edgeArray, nodeCount,
                               dim, learning_rate,
                               epoch, gpu_fraction,
                               batchsize, print_every_epoch,
                               save_cp=save_checkpoints)

        print('Emb shape', embeddings.shape)

        self.vectors = {}
        for n in range(nodeCount):
            self.vectors[graph.look_back_list[n]] = embeddings[n, :]
